...
public final class ISO8601Converter {
    ...
    public static XMPDateTime parse(String iso8601String, XMPDateTime binValue) throws XMPException {
        ...
        boolean timeOnly = input.ch(0) != 'T' ? (input.length() < 2 || input.ch(1) != ':') ? input.length() >= 3 && input.ch(2) == ':' : true : true;
        if (timeOnly) {
            binValue.setMonth(1);
            binValue.setDay(1);
        } else {
    ...
    public static String render(XMPDateTime dateTime) {
        ...
        if (dateTime.getHour() == 0 && dateTime.getMinute() == 0 && dateTime.getSecond() == 0 && dateTime.getNanoSecond() == 0) {
            if (!(dateTime.getTimeZone() == null || dateTime.getTimeZone().getRawOffset() == 0)) {
            }
            return buffer.toString();
        }
        buffer.append('T');
