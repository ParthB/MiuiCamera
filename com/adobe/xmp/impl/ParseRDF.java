...
public class ParseRDF {
                ...
                for (i = 0; i < attributes.getLength(); i++) {
                    ...
                    if (!"xml:lang".equals(attribute.getNodeName()) || ("ID".equals(attrLocal) && "http://www.w3.org/1999/02/22-rdf-syntax-ns#".equals(attrNS))) {
                        if ("datatype".equals(attrLocal) && "http://www.w3.org/1999/02/22-rdf-syntax-ns#".equals(attrNS)) {
                            ...
                            if (!("parseType".equals(attrLocal) ? "http://www.w3.org/1999/02/22-rdf-syntax-ns#".equals(attrNS) : false)) {
                                rdf_EmptyPropertyElement(xmp, xmpParent, xmlNode, isTopLevel);
    ...
    private static void fixupQualifiedNode(XMPNode xmpParent) throws XMPException {
        ...
        if (!-assertionsDisabled) {
            if (!(xmpParent.getOptions().isStruct() ? xmpParent.hasChildren() : false)) {
        ...
        if (-assertionsDisabled || "rdf:value".equals(valueNode.getName())) {
            ...
            for (i2 = 1; i2 <= valueNode.getQualifierLength(); i2++) {
                xmpParent.addQualifier(valueNode.getQualifier(i2));
            }
            for (i2 = 2; i2 <= xmpParent.getChildrenLength(); i2++) {
                xmpParent.addQualifier(xmpParent.getChild(i2));
            }
            if (!-assertionsDisabled) {
                if (!xmpParent.getOptions().isStruct()) {
                    i = xmpParent.getHasValueChild();
                }
                if (i == 0) {
                    throw new AssertionError();
                }
