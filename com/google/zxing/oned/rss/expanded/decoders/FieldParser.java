    ...
    private static String processFixedAI(int aiSize, int fieldSize, String rawInformation) throws NotFoundException {
        if (rawInformation.length() >= aiSize) {
            ...
            if (rawInformation.length() >= aiSize + fieldSize) {
                ...
                String result = new StringBuilder(String.valueOf('(')).append(ai).append(')').append(field).toString();
                String parsedAI = parseFieldsInGeneralPurpose(remaining);
                return parsedAI != null ? new StringBuilder(String.valueOf(result)).append(parsedAI).toString() : result;
            } else {
                throw NotFoundException.getNotFoundInstance();
            }
        }
