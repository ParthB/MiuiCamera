...
public class V6GestureRecognizer {
    ...
    private class MyEdgeGestureListener implements EdgeGestureListener {
        ...
        public boolean onEdgeTap(int x, int y) {
            android.util.Log.v("CameraGestureRecognizer", "MyEdgeGestureListener onEdgeTap x=" + x + " y=" + y);
            if (V6GestureRecognizer.this.mActivity.getCurrentModule().IsIgnoreTouchEvent() || !V6GestureRecognizer.this.getUIController().getPreviewPage().isPreviewPageVisible() || !V6ModulePicker.isCameraModule()) {
                return false;
            }
            android.util.Log.v("CameraGestureRecognizer", "EdgeGesture detectTap sucess");
            return V6GestureRecognizer.this.getUIController().getEdgeShutterView().onEdgeTap(x, y);
        }

        public boolean onEdgeTouch(int x, int y) {
            android.util.Log.v("CameraGestureRecognizer", "MyEdgeGestureListener onEdgeTouch x=" + x + " y=" + y);
            if (V6GestureRecognizer.this.mActivity.getCurrentModule().IsIgnoreTouchEvent() || !V6GestureRecognizer.this.getUIController().getPreviewPage().isPreviewPageVisible() || !V6ModulePicker.isCameraModule()) {
                return false;
            }
            android.util.Log.v("CameraGestureRecognizer", "EdgeGesture detectTouch sucess");
            return V6GestureRecognizer.this.getUIController().getEdgeShutterView().onEdgeTouch(x, y);
        }
    }
