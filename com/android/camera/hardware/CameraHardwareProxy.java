...
public class CameraHardwareProxy {
    ...
    public boolean isNeedFlashOn(Camera camera) {
        Object ret = ReflectUtil.callMethod(camera.getClass(), camera, "getFlashOn", "()I", new Object[0]);
        if (ret != null && ((Integer) ret).intValue() == 1) {
            return true;
        }
        return false;
    }
    ...
    public void setMetadataCb(Camera camera, CameraMetaDataCallback cb) {
        if (CameraSettings.isSupportedMetadata()) {
            ...
            ReflectUtil.callMethod(camera.getClass(), camera, "setMetadataCb", "(Landroid/hardware/Camera$CameraMetaDataCallback;)V", callbackProxy);
        }
    }
