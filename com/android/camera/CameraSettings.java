...
public class CameraSettings {
    ...
    public static final int PREVIEW_FRAME_TOP_MARGIN = CameraAppImpl.getAndroidContext().getResources().getDimensionPixelSize(R.dimen.preview_frame_top_margin);
    ...
    public static boolean isNearRatio16_9(int width, int height) {
        ...
        if (Math.abs(ratio - 1.3333333333333333d) > Math.abs(ratio - 1.7777777777777777d) || Math.abs(ratio - 1.5d) < 0.02d) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isScanQRCode(SharedPreferences pref) {
    ...
    private static void changePreviewFrameLayoutParams(ActivityBase activity, int index) {
        ...
        if (index == 0) {
            p1.setMargins(0, PREVIEW_FRAME_TOP_MARGIN, 0, 0);
        } else {
            p1.setMargins(0, 0, 0, 0);
        }
    ...
    public static void resetSettingsNoNeedToSave(CameraSettingPreferences preferences, int cameraId) {
        ...
        editor.remove("pref_camera_ae_bracket_hdr_key");
    ...
    public static void setEdgeMode(Context context, boolean enable) {
        ...
        if (context != null) {
            ...
            if (isEdgePhotoEnable()) {
                ...
                ReflectUtil.callMethod(cls, im, str, str2, objArr);
            }
        }
    }
