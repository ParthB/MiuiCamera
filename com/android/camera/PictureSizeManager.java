...
public class PictureSizeManager {
    ...
    public static String[] getEntries() {
        if (Device.is18x9RatioScreen()) {
            return new String[]{CameraSettings.getString(R.string.pref_camera_picturesize_entry_standard), CameraSettings.getString(R.string.pref_camera_picturesize_entry_widescreen), CameraSettings.getString(R.string.pref_camera_picturesize_entry_fullscreen)};
        }
    ...
    public static void initialize(ActivityBase mActivity, List<Size> supported, int limit) {
        ...
        PictureSize size18_9 = findMaxRatio18_9(supported);
        if (size18_9 != null) {
    ...
    private static PictureSize _findMaxRatio18_9(List<PictureSize> supported) {
        int maxWidth = 0;
        int maxHeight = 0;
        for (PictureSize size : supported) {
            if (CameraSettings.isAspectRatio18_9(size.width, size.height) && size.width * size.height > maxWidth * maxHeight) {
                maxWidth = size.width;
                maxHeight = size.height;
            }
        }
    ...
    public static PictureSize findMaxRatio18_9(List<Size> supported) {
        int maxWidth = 0;
        int maxHeight = 0;
        for (Size size : supported) {
            if (CameraSettings.isAspectRatio18_9(size.width, size.height) && size.width * size.height > maxWidth * maxHeight) {
