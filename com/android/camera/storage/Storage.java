...
public class Storage {
    ...
    public static void initStorage(Context context) {
        if (Device.isSupportedSecondaryStorage()) {
            if (VERSION.SDK_INT >= 23) {
                ...
                try {
                    Object vols = ReflectUtil.callMethod(storageManager.getClass(), storageManager, "getVolumes", "()Ljava/util/List;", new Object[0]);
                    Class<?> volumeInfoClazz = Class.forName("android.os.storage.VolumeInfo");
                    Object sdcardVolume = null;
                    if (vols != null && (vols instanceof List)) {
                        int typePublic = ReflectUtil.getFieldInt(volumeInfoClazz, null, "TYPE_PUBLIC", -1);
                        Class<?> diskInfoClazz = Class.forName("android.os.storage.DiskInfo");
                        for (Object vol : (List) vols) {
                            Object typeRet = ReflectUtil.callMethod(volumeInfoClazz, vol, "getType", "()I", new Object[0]);
                            if (typeRet != null && ((Integer) typeRet).intValue() == typePublic) {
                                Object writable = ReflectUtil.callMethod(volumeInfoClazz, vol, "isMountedWritable", "()Z", new Object[0]);
                                if (writable != null && ((Boolean) writable).booleanValue()) {
                                    Object disk = ReflectUtil.callMethod(volumeInfoClazz, vol, "getDisk", "()Landroid/os/storage/DiskInfo;", new Object[0]);
                                    if (disk != null) {
                                        Object isSd = ReflectUtil.callMethod(diskInfoClazz, disk, "isSd", "()Z", new Object[0]);
                                        if (isSd != null && ((Boolean) isSd).booleanValue()) {
                                            sdcardVolume = vol;
                                            break;
                                        }
                                    }
                    ...
                    if (sdcardVolume != null) {
                        Object file = ReflectUtil.callMethod(volumeInfoClazz, sdcardVolume, "getPath", "()Ljava/io/File;", new Object[0]);
                        String sdcardPath = file == null ? null : ((File) file).getPath();
                        if (sdcardPath != null) {
    ...
    public static Uri addImage(Context context, String title, long date, Location location, int orientation, byte[] jpeg, int width, int height, boolean mirror, boolean isHide, boolean isMap, boolean appendExif) {
        return addImage(context, title, date, location, orientation, jpeg, width, height, mirror, isHide, isMap, appendExif, false);
    }

    public static Uri addImage(Context context, String title, long date, Location location, int orientation, byte[] jpeg, int width, int height, boolean mirror, boolean isHide, boolean isMap, boolean appendExif, boolean isThumb) {
        return addImage(context, title, date, location, orientation, jpeg, width, height, mirror, isHide, isMap, appendExif, isThumb, false);
    }

    public static Uri addImage(Context context, String title, long date, Location location, int orientation, byte[] jpeg, int width, int height, boolean mirror, boolean isHide, boolean isMap, boolean appendExif, boolean isThumb, boolean isPortrait) {
        Exception e;
                ...
                try {
                    ...
                    try {
                        ...
                        if (isMap) {
                            ...
                            title = title.substring(0, focusSuccess ? title.lastIndexOf("_") : title.lastIndexOf("_UBIFOCUS_"));
                            path = generateFilepath(title, false, false);
                            new File(generateFilepath(title + (focusSuccess ? "_" : "_UBIFOCUS_") + centerFocused, isHide, false)).renameTo(new File(path));
                            if (!focusSuccess) {
                                deleteImage(title);
                            }
                        }
    ...
    public static boolean updateImage(Context context, byte[] jpeg, ExifInterface exif, Uri uri, String title, Location location, int orientation, int width, int height, String oldTitle, boolean isPortrait) {
