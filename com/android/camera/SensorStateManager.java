...
public class SensorStateManager {
    ...
    public boolean isStable() {
        ...
        if (this.mAngleSpeedIndex >= 0) {
            double total = 0.0d;
            for (double speed : this.mAngleSpeed) {
                total += speed;
            }
            double avgSpeed = total / ((double) this.mAngleSpeed.length);
            double lastSpeed = this.mAngleSpeed[Util.clamp(this.mAngleSpeedIndex, 0, this.mAngleSpeed.length - 1)];
            Log.v("SensorStateManager", "isStable mAngleSpeed=" + avgSpeed + " lastSpeed=" + lastSpeed + " threshold=" + GYROSCOPE_STABLE_THRESHOLD);
            if (avgSpeed > GYROSCOPE_STABLE_THRESHOLD || lastSpeed > GYROSCOPE_STABLE_THRESHOLD) {
                z = false;
            }
            return z;
        }
        Log.v("SensorStateManager", "isStable return true for mAngleSpeedIndex=" + this.mAngleSpeedIndex + " threshold=" + GYROSCOPE_STABLE_THRESHOLD);
        return true;
    }
