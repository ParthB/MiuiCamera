...
public abstract class BaseModule implements Module, ExposureViewListener, HardwareErrorListener {
            ...
            switch (msg.what) {
                case 0:
                    if (BaseModule.this.mDisplayRotation != 180) {
                        BaseModule.this.mActivity.getScreenHint().showFrontCameraFirstUseHint();
                        CameraSettings.cancelFrontCameraFirstUseHint(BaseModule.this.mPreferences);
                        return;
                    }
                    return;
                case 1:
                    BaseModule.this.mActivity.getScreenHint().dismissFrontCameraFirstUseHint();
                    return;
                default:
                    return;
            }
    ...
    private boolean mIgnoreTouchEvent = true;
    ...
    public boolean handleMessage(int what, int sender, Object extra1, Object extra2) {
        switch (sender) {
            case R.id.zoom_button:
                if (!this.mPaused && isCameraEnabled() && CameraSettings.isSupportedOpticalZoom() && what == 7) {
                    if (!((Boolean) extra2).booleanValue() || !CameraSettings.isSwitchCameraZoomMode()) {
                        if (!((Boolean) extra2).booleanValue()) {
                            this.mParameters.set("optical-zoom-to-tele", "true");
    ...
    public boolean dispatchEdgeTouchEvent(MotionEvent event) {
        return V6GestureRecognizer.getInstance(this.mActivity).onEdgeTouchEvent(event);
    }
    ...
    protected void hideFrontCameraFirstUseHint() {
        if (this.mDisplayRotation == 180 && isFrontCamera() && this.mActivity.getScreenHint().isShowingFrontCameraFirstUseHint() && !this.mHandler.hasMessages(1)) {
    ...
    public boolean isInTapableRect(int x, int y) {
        ...
        return tapableRectWithEdgeSlop().contains(point.x, point.y);
    }
    ...
    private Rect tapableRectWithEdgeSlop() {
        int edgeSlop;
        Rect rect = new Rect(this.mActivity.getCameraScreenNail().getRenderRect());
        if (getUIController().getPreviewFrame().isFullScreen()) {
    ...
    public void onZoomValueChanged(int value, boolean sync) {
        if (!this.mPaused && this.mParameters != null && this.mCameraDevice != null && isCameraEnabled()) {
            if (this.mParameters.getZoom() == 0 && value > 0) {
                CameraDataAnalytics.instance().trackEvent("zoom_times_key");
            }
            setZoomValue(value);
    ...
    protected void changePreviewSurfaceSize() {
        ...
        switch (this.mUIStyle) {
            ...
            case 1:
                width = Util.sWindowWidth;
                height = (Util.sWindowWidth * 16) / 9;
                break;
            case 3:
                width = Util.sWindowWidth;
                height = (Util.sWindowWidth * 18) / 9;
                break;
        }
    ...
    protected void updateCameraScreenNailSize(int width, int height, FocusManagerAbstract focusManager) {
        ...
        if (this.mActivity.getCameraScreenNail().getWidth() == width && this.mActivity.getCameraScreenNail().getHeight() == height && this.mActivity.getCameraScreenNail().getRenderTargeRatio() == CameraSettings.getRenderAspectRatio(width, height) && !this.mSwitchingCamera) {
