...
public abstract class BasePreferenceActivity extends PreferenceActivity implements OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        } else if ("pref_camera_facedetection_key".equals(preference.getKey())) {
            if (CameraSettings.isSupportedPortrait() && CameraSettings.isBackCamera() && V6ModulePicker.isCameraModule() && this.mFaceDetectionHitCountDown > 0) {
        ...
        } else if ("pref_scan_qrcode_key".equals(preference.getKey()) && !CameraSettings.isQRCodeReceiverAvailable(this)) {
    ...
    private void installQRCodeReceiver() {
        new AsyncTask<Void, Void, Void>() {
            protected Void doInBackground(Void... params) {
                Log.d(BasePreferenceActivity.TAG, "install...");
    ...
    private void updateQRCodeEntry() {
        ...
        if (scanQRCode != null && this.mPreferences.getBoolean("pref_scan_qrcode_key", scanQRCode.isChecked()) && !CameraSettings.isQRCodeReceiverAvailable(this)) {
            Log.d(TAG, "disable QRCodeScan");
            Editor editor = this.mPreferences.edit();
