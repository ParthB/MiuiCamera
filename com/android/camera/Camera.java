...
public class Camera extends ActivityBase implements OnRequestPermissionsResultCallback {
    ...
    public void onResume() {
        boolean z;
        if (startFromKeyguard() && !PermissionManager.checkCameraLaunchPermissions()) {
            finish();
        }
        if (Util.checkDeviceHasNavigationBar(this)) {
        ...
        MiStatInterface.recordPageStart(this, "CameraActivity");
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (!super.dispatchTouchEvent(event)) {
            z = this.mCurrentModule.dispatchTouchEvent(event);
        }
        return z;
    }
    ...
    public boolean onEdgeTouchEvent(MotionEvent event) {
        if (Device.isSupportedEdgeTouch()) {
            return this.mCurrentModule.dispatchEdgeTouchEvent(event);
        }
        return true;
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if (requestCode == PermissionManager.getCameraRuntimePermissionRequestCode() && !PermissionManager.isCameraLaunchPermissionsResultReady(permissions, grantResults)) {
            finish();
        }
    }
    ...
    private void bindGalleryService() {
        try {
            ...
            Log.w("Camera", "bindGalleryService error.", e);
        }
    }
