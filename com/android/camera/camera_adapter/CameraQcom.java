...
public class CameraQcom extends CameraModule {
    ...
    private void qcomUpdateCameraParametersPreference() {
        ...
        if (CameraSettings.isSwitchOn("pref_camera_portrait_mode_key")) {
            ...
            Log.v("Camera", "PortraitMode value = on");
        } else {
            sProxy.setPortraitMode(this.mParameters, "off");
            Log.v("Camera", "PortraitMode value = off");
        }
        ...
        if (!(this.mMutexModePicker.isNormal() || this.mMultiSnapStatus)) {
    ...
    private String getZSL() {
        ...
        if (Device.isFrontRemosicSensor() && isFrontCamera() && getString(R.string.pref_face_beauty_close).equals(CameraSettings.getFaceBeautifyValue())) {
            return "off";
        }
    ...
    protected boolean isAutoRestartInNonZSL() {
        return Device.isFrontRemosicSensor() ? isFrontCamera() : false;
    }
}
